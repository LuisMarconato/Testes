...
def SALVAR_SENHA():
    ...
    senha_adm= "\033[1m"+interface.Label_senha.get()+"\033[0m"
    #SOMENTE PARA VER SE TA SALVANDO 
    print(interface.Label_senha.get())
    #1.1.2- DESABILITA O LABEL DE SENHA EO BOTÃO ***(não esta pronto!)
    interface.Label_senha.configure(state=DISABLED)
    interface.salva_senha.configure(state=DISABLED)    

#1.2 - PARAR/INICIAR O GUARD
def PARAR_GUARD():
    win32serviceutil.StopService("SISTEMAGuard_serv")
def INICIAR_GUARD():
    win32serviceutil.StartService("SISTEMAGuard_serv")
...
def verifica_servicos_guard():
    verifica_guard= win32serviceutil.QueryServiceStatus('SISTEMAGuard_serv')[1]
    if verifica_guard ==1:
        return False
...
def TROCA():
    ...
    combo = interface.cb_modulos.get()
    if combo == interface.lista_modulos[0]:
        modulo = interface.caminho_pro
    elif combo == interface.lista_modulos[1]:
        modulo = interface.caminho_slim
    elif combo == interface.lista_modulos[2]:
        modulo = interface.caminho_mei
    ...
    if verifica_servicos_firebird()==False and verifica_servicos_guard()==False:
         ...
         #1.5.5- COPIA TUDO DA PASTA BASE CLIENTE PARA A PASTA PASTA SISTEMA.
    ...
    if verifica_servicos_firebird()==True:
        ...
        cur.execute(f"update usuarios set senha = '{interface.Label_senha.get()}' where usuario = 'ADMINISTRADOR'")
        conecta.commit()
        
        #1.5.4.2- INICIAR O GUARD PARA CONSEGUIR ABRIR O SISTEMA 
        INICIAR_GUARD()      
        
        #1.5.4.3- APRESENTAR MENSAGEM INDICADO STATUS
        status = 'Troca de base executada com sucesso!'
        senha = f'Senha do ADM alterada para: {interface.Label_senha.get()}'
        interface.texto_status["text"] = status
        interface.texto_senha["text"] = senha

        #1.5.4.4- HABILITANDO O BOTÃO REVERTER E DESABILITANDO O BOTÃO TROCA
        interface.TROCA_button.configure(state='disable')
        interface.REVERTE_button.configure(state='normal')
...
def REVERTER():
    ...
    if verifica_servicos_firebird()==False and verifica_servicos_guard()==False:    
        ...
        #1.6.5- MOVE O DATAGES DA PASTA SISTEMA PARA A PASTA BASE CLIENTE
        ...
        #1.6.7- MOVE O DATAGES DO TECNICO PARA A PASTA DO SISTEMA PRO
    ...
    if verifica_servicos_guard()==False: 
        ...
        #1.6.6- INICIAR O GUARD PARA CONSEGUIR ABRIR O SISTEMA 
        INICIAR_GUARD()
        ...
        interface.TROCA_button.configure(state=NORMAL)
        interface.REVERTE_button.configure(state=DISABLED)
        interface.Label_senha.configure(state=NORMAL)
        interface.salva_senha.configure(state=NORMAL)
...
def DELETE_VERSAO():
    ...
    #1.7.4- INICIAR O GUARD PARA CONSEGUIR ABRIR O SISTEMA
    if verifica_servicos_guard()== False:
        INICIAR_GUARD()